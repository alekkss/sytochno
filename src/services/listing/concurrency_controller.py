...
- При низком проценте ошибок — плавно увеличивает лимит (+1).
- При высоком проценте ошибок — резко снижает (÷2) и ставит cooldown.
...
# 30 секунд — достаточно для статистически значимой выборки
# при 20+ одновременных воркерах (каждый делает запрос ~3-5 сек).
...
_ERROR_RATE_LOW: float = 0.10  # 10%

# Порог ошибок для снижения лимита (верхняя граница «жёлтой зоны»).
# Выше этого — снижаем лимит вдвое и включаем cooldown.
_ERROR_RATE_HIGH: float = 0.30  # 30%
...
_INCREASE_STEP: int = 1

# Минимальный интервал между увеличениями лимита (секунды).
# Не чаще одного раза в 30 секунд — даём системе стабилизироваться.
_INCREASE_INTERVAL_SECONDS: float = 30.0

# Длительность cooldown после снижения лимита (секунды).
# Все воркеры замирают на это время — даём WAF сайта «забыть» наши IP.
...
class ConcurrencyController:
    ...
    def _try_decrease(self, now: float) -> None:
        ...
        При срабатывании:
        - Лимит делится на 2 (но не ниже floor).
