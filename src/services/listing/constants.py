...
# Таймаут навигации page.goto для страниц карточек (миллисекунды).
# Используется как дефолт, если navigation_timeout не передан
# в PageLoader через конструктор. 45 секунд — компромисс между
# скоростью и стабильностью через медленные прокси: стандартные
# 30 секунд слишком мало для SPA через резидентные прокси,
# а полные 60 секунд задерживают каждый retry при бане IP.
DEFAULT_GOTO_TIMEOUT_MS: int = 45000
...
# ещё ДО domcontentloaded. Но через медленные прокси (резидентные,
# ротируемые) JS-бандл может грузиться значительно дольше.
# 10 секунд — запас для медленных прокси без бесполезного ожидания
# networkidle (который тянет 10+ секунд даже при быстром соединении).
# Поллинг каждые 0.2 секунды обеспечивает мгновенный выход при
# перехвате — при быстрой прокси реальное ожидание 0–1 секунда.
TOKEN_WAIT_AFTER_DOM_SECONDS: float = 10.0
...
# после перехвата, не дожидаясь полных TOKEN_WAIT_AFTER_DOM_SECONDS.
TOKEN_POLL_INTERVAL_SECONDS: float = 0.2
